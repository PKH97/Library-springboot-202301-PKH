...
// @RequiredArgsConstructor 를 사용하면 @Autowired 가 아닌 final 이 그 역할을 한다.
    private final RentalService rentalService;

    /*  /rental/{bookId}
        대여요청 -> 대여요청 날린 사용자의 대여가능 여부확인
        -> 가능함(대여 가능 횟수 3권 미만일때) -> 대여 정보 추가 rental_mst(대여코드), rental_dtl
        -> 불가능함(대여 가능 횟수 0일때) -> 예외처리 */
    ...
    public ResponseEntity<CMRespDto<?>> rental(@PathVariable int bookId,
        ...
        return ResponseEntity
                .ok()
                .body(new CMRespDto<>(HttpStatus.OK.value(), "Successful", null));
    }
}
