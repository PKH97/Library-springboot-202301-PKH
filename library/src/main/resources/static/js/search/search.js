        ...
        responseData.forEach((data, index) => {
            ...
            if(principal == null) {
                ...
                if(data.rentalDtlId != 0 && data.returnDate == null && data.userId != principal.user.userId) {
                    ...
                    bookButtons[bookButtonsLength + index].innerHTML = `
                        <button type="button" class="rental-buttons return-button">대여하기</button> 
                    `;
                }
...
class ComponentEvent {
    ...
    addClickEventCategoryCheckboxs() {
        ...
        checkboxs.forEach(checkbox => {
            checkbox.onclick = () => {
                ...
                document.querySelector('.search-button').click(); //체크박스 클릭시 바로 정보 들고옴
            }
        });
    }
    ...
    addClickEventRentalButton() {
        const rentalButtons = document.querySelectorAll('.renatl-buttons');
        const bookIds = document.querySelectorAll('.book-id');
